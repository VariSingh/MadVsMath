...
// escape variables for security
    ...
    if (mysqli_query($con, "INSERT INTO  mastermath.eachgame ( fbid ,string, uagent, ip ) VALUES ( '$userid' , '$timestring', '$_SERVER[HTTP_USER_AGENT]', '$_SERVER[REMOTE_ADDR]' )")) {
        $result = "datainserted";
    } else {
        $result = "query error: " . mysqli_error($con);
    }
    $query = mysqli_query($con, "select maxscore from bande where fbid='$userid'");
    if ($query) {
        while ($row = mysqli_fetch_array($query)) {
            //$row['maxscore'];
//$result = "userscore and maxscore".$userscore."<>".$row['maxscore']; 
            $result = "next query3 ".$userscore." ".$row['maxscore'];
            if ($userscore>$row['maxscore']) {
            $result = "next query4 ".$userscore." ".$row['maxscore'];
                if (mysqli_query($con, "UPDATE bande SET maxscore='$userscore',maxlevel='$umaxlevel',candies='$candies' WHERE fbid='$userid'")) {
